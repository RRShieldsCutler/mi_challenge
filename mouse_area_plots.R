...
m.samples <- as.character(map[map$mouse==mouse.ids[1],]$sampleid)
otu.t <- otu.L3[,m.samples]
map.t <- map[map$mouse==mouse.ids[1],]
...
ggplot(otu.m.area, aes(x = exp_day_n, y = RelativeAbundance, fill = Taxa)) +
  ...
  scale_fill_manual(values=c("dodgerblue2","#E31A1C", # red # Kevin Wright
                             ...
                             "darkorange4","brown")) +
  theme_classic() + theme(axis.text = element_text(color="black"))
ggsave(filename = paste0("../../results/mouse_plots_by_animal_",mouse.ids[1],"_L3.png"), height = 6, width = 7, dpi = 300)
